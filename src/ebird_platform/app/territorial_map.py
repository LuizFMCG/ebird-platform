...
def desenhar_mapa_conesul() -> None:
    st.subheader("Mapa territorial â€” riqueza de espÃ©cies por Estado/MunicÃ­pio")
    st.caption("Selecione paÃ­ses e a escala territorial para visualizar a riqueza de espÃ©cies com base em registros do eBird.")
    ...
    if not paises_permitidos:
        st.error("NÃ£o encontrei paÃ­ses da AmÃ©rica do Sul nas dimensÃµes territoriais (coluna pais_iso3).")
        return
    ...
    nivel = st.radio("Escala territorial", options=["Estados", "MunicÃ­pios"], horizontal=True)

    paises_est_set = set(dim_est["pais_iso3"].dropna().unique())
    paises_mun_set = set(dim_mun["pais_iso3"].dropna().unique())

    if nivel == "MunicÃ­pios":
        missing = [p for p in paises_sel if p not in paises_mun_set]
        if missing:
            st.warning("Sem geometria municipal para: " + ", ".join(missing) + ". Eles serÃ£o ignorados no mapa de municÃ­pios.")
        paises_sel = [p for p in paises_sel if p in paises_mun_set]
    else:
        missing = [p for p in paises_sel if p not in paises_est_set]
        if missing:
            st.warning("Sem geometria estadual para: " + ", ".join(missing) + ". Eles serÃ£o ignorados no mapa de estados.")
        paises_sel = [p for p in paises_sel if p in paises_est_set]

    if not paises_sel:
        st.warning("Selecione ao menos um paÃ­s.")
        return
    ...
    if nivel == "MunicÃ­pios":
        ...
        id_col = "id_municipio"
        tooltip_aliases = ["MunicÃ­pio:", "Riqueza (nÂº de espÃ©cies):"]
    else:
        ...
        id_col = "id_estado"
        tooltip_aliases = ["Estado:", "Riqueza mÃ¡xima (municÃ­pios):"]

    if gdf.empty:
        st.warning("NÃ£o hÃ¡ dados para o paÃ­s/escala selecionados.")
        return

    if metric_col not in gdf.columns:
        st.error(f"Coluna de mÃ©trica '{metric_col}' nÃ£o encontrada no GeoDataFrame.")
        st.write("Colunas disponÃ­veis:", list(gdf.columns))
        return
